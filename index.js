...
function processContains(item, list, callback) {
  ...
  let match = list.find((element) => element === item);
...
function getFullNames(runners) {
  ...
  fullNames.push(`${currentItem.last_name}, ${currentItem.first_name}`);
  })
  return fullNames;
}
...
function firstNamesAllCaps(runners) {
  let map = runners.map(currentItem => currentItem.first_name.toUpperCase());
  return map; 
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let myArray = runners.filter(x => x == tShirtSize);
  return myArray;
}

getRunnersByTShirtSize(runnersList, "S");
...
function tallyUpDonations(runners) {
  ...
  let donations = []
  runners.forEach(function(currentItem) {
    donations.push(currentItem.donation);
  })


  const reducer = (accumulator, currentValue) => accumulator + currentValue;
  return donations.reduce(reducer);
}

tallyUpDonations(runnersList);
 ...
 * etc
*/
function counterMakerWithLimit(maxNumber) {
  let count = 0;
  return function counter() {
    if (count > maxNumber) {
      count = 0;
